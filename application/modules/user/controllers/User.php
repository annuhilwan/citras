...
class User extends CI_Controller {
    ...
    //start enchance phase1 req 14
    public function generateresetPassword($id){
        is_login();
        ...
        $this->User_model->resetpassWordDefault($id);

        $datapasien= $this->User_model->get_data_email($id);
    /*    
        ...
        $this->email->to($datapasien['email']);
        //$this->email->to('rezapawanov@gmail.com');
        $this->email->subject('Reset Password Citra');
        $this->email->message('Your Citra Password has been reset to your birthday date with format DDMMYYYY (example : 17081945).');
...
*/
       ...
       // SMTP configuration
       $mail->isSMTP();
       $mail->Host     = 'mail.mastercoding.id';
       $mail->SMTPAuth = true;
       $mail->Username = 'citra@mastercoding.id';
       $mail->Password = 'citratesting123';
       $mail->SMTPSecure = 'tls';
       $mail->Port     = 587;

       $mail->setFrom('citra@mastercoding.id', 'Medical Citra');
       $mail->addReplyTo('citra@mastercoding.id', 'Medical Citra');

       // Add a recipient
      $mail->addAddress($datapasien['email']);
       ...
       // Email subject
       $mail->Subject = 'Medical Citra Password Reset';

       // Set email format to HTML
       $mail->isHTML(true);

       // Email body content
       $mailContent = "<h1>Your Citra Password has been reset</h1>
           <p>Please use your birthday date as your password with format DDMMYYYY (example : 17081945).</p>
           <p>Thank You</p>";
    ...
    //end enchance phase1 req 14
    
    public function generateresetPasswordBasic($id){
        is_login();
    ...
    public function dataTable (){
        is_login();
	    $table = "(SELECT a.users_id, a.user_id, a.status, a.name, a.email, a.user_type, b.nama_pt as perusahaan, b.bagian, b.no_identitas as nik 
	    FROM users a LEFT JOIN master_pasien b ON a.id_pasien = b.id) temp";
    	$primaryKey = 'users_id';
    	$columns = array(
			...
			array( 'db' => 'email', 'dt' => 3 ),
			array( 'db' => 'nik', 'dt' => 4 ),
			array( 'db' => 'bagian', 'dt' => 5 ),
			array( 'db' => 'perusahaan', 'dt' => 6 ),
		...
		foreach ($output_arr['data'] as $key => $value) {
			...
			$output_arr['data'][$key][count($output_arr['data'][$key])  - 1] .= '<a style="cursor:pointer;" data-toggle="modal" class="mClass" onclick="setId('.$id.', \'user\')" data-target="#cnfrm_delete" title="deletes"><i class="fa fa-trash-o" ></i></a>';}
    ...
    public function updateStatus($id){
        ...
        $ids = explode('-', $id);
        foreach ($ids as $id) {
            $this->User_model->updateStatus($id); 
        }
       redirect(base_url().'user/userTable', 'refresh');
    }
    
    public function updateStatusDel($id){
        is_login(); 
        $ids = explode('-', $id);
        foreach ($ids as $id) {
            $this->User_model->updateStatusDel($id); 
        }
       redirect(base_url().'user/userTable', 'refresh');
    }
    
    public function updatePassBasic($id){
        is_login(); 
        $ids = explode('-', $id);
        foreach ($ids as $id) {
            $this->User_model->updatePassBasic($id); 
        }
       redirect(base_url().'user/userTable', 'refresh');
    }
