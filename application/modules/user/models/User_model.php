...
class User_model extends CI_Model {       
	...
	//start enhance phase 1 req 16
	function resetpassWordDefault($id){
		$res4 = $this->db->query("select tanggal_lahir from master_pasien where id in (select id_pasien from users where users_id  = '".$id."');")->row_array();
		//$defaultpass = date_format($res4['tanggal_lahir'],"dmY");
		$timestamp1 = strtotime($res4['tanggal_lahir']);
		$defaultpass = date("dmY", $timestamp1);
		$password = password_hash($defaultpass, PASSWORD_DEFAULT);
		$res = $this->db->query("update users set password = '".$password."' where users_id = '".$id."';");				

	}

	function get_data_email($id){
		$res = $this->db->query("select email from users where users_id  = '".$id."';")->row_array();
		return $res;
	}

	//end enhance phase 1 req 16
	...
	function updateStatusDel($id='') {
  	    $data['status'] = 'deleted';
		$this->db->where('users_id', $id);  
		$this->db->update('users', $data); 
	}
	
	function updatePassBasic($id='') {
		$res4 = $this->db->query("select tanggal_lahir from master_pasien where id in (select id_pasien from users where users_id  = '".$id."');")->row_array();
		$defaultpass = date("dmY");
		if ($res4 !== null) {
    		$timestamp1 = strtotime($res4['tanggal_lahir']);
    		$defaultpass = date("dmY", $timestamp1);
		}
