...
class Master_model extends CI_Model {       
	...
	function get_data_pasien($idPaket){
		$res = $this->db->query("SELECT id, nama_paket, harga_paket, tipe_pemeriksaan FROM master_paket_medcheck_header where id = ".$idPaket."")->row_array();
		return $res;
	}
	
	function get_data_paket_pasien($name){
		$res = $this->db->query("SELECT id, nama_paket, tipe_pemeriksaan FROM master_paket_medcheck_header where nama_paket = '".$name."'")->row_array();
		return $res;
	}
	...
	function get_data_master_tipe_pemeriksaan(){
		$res = $this->db->query("SELECT * FROM master_tipe_pemeriksaan where status = 'ACTIVE'")->result_array();
		return $res;
	}
	
	function get_data_tipe_pemeriksaan_pasien($id){
		$res = $this->db->where_in('id', $id)->get('master_tipe_pemeriksaan');
        return $res->result_array();
	}
	...
	public function updateRow($table, $col, $colVal, $data) {
  	...
  	function save_pemeriksaan_label_check_add($post){
		$data['tipe_pemeriksaan'] = $post['label_id'];
		$this->db->where('id', $post['id_paket']);
		$res = $this->db->update('master_paket_medcheck_header', $data);
		return $res;
  	}
