...
class Master extends CI_Controller {
    ...
    public function get_modal_tipe_pemeriksaan_check() {
        is_login();
        $id_paket = $this->input->post('paket_id');
        $arrDataPaket = $this->Master_model->get_data_pasien($id_paket);
		$arrTipePemeriksaan = $this->Master_model->get_data_master_tipe_pemeriksaan();
        echo $this->load->view('modal_tipe_pemeriksaan',  Array('arrTipePemeriksaan'=>$arrTipePemeriksaan, 'arrDataPaket'=>$arrDataPaket), true);
        exit;
    }
    
    public function save_pemeriksaan_label_check_add() {
        is_login();
        $post = $this->input->post();
        $res = $this->Master_model->save_pemeriksaan_label_check_add($post);
		echo 1;
		exit;
    }
